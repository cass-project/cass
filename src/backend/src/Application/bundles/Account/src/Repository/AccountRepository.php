<?php
namespace Application\Account\Repository;
...
class AccountRepository extends EntityRepository
{
    public function findByEmail($email) : Account
    {
        try {
            ...
            throw new AccountNotFoundException(sprintf('Application\Account with email `%s` not found', $email));
        }
    }
    ...
    public function findByAPIKey(string $apiKey)
    {
        ...
        if($account === null) {
            throw new AccountNotFoundException(sprintf('Application\Account with api_key `%s` not found', $apiKey));
        }
